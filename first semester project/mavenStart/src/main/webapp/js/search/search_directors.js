...
function ajaxSearch(){
    var queryDirector = document.getElementById('searchInput').value;
    queryDirector = queryDirector.trim();
        var searchType = document.getElementById('searchType').value;
        var url = "/searchDirectors?director=" + queryDirector + "&"
            + "searchType=" + searchType + "&"
            + "numPage=" + numPage.val();

        $.get(url, function (responseXml) {
            if(pastQuery !== queryDirector){
                numPage.val(0);
            }
            pastQuery = queryDirector;
