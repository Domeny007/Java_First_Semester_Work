...
function ajaxSearch(){
    ...
    var queryActor = document.getElementById('searchInput').value;
    queryActor = queryActor.trim();
    var searchType = document.getElementById('searchType').value;
    var url = "/searchActors?actor=" + queryActor + "&"
        + "searchType=" + searchType + "&"
        + "numPage=" + numPage.val();

    $.get(url, function (responseXml) {
        if(pastQuery !== queryActor){
            numPage.val(0);
        }
        pastQuery = queryActor;
